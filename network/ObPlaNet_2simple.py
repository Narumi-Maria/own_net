"""该模型对应将前景pool之后复制与背景特征concat的方法"""
...
class ObPlaNet_resnet18(nn.Module):
    def __init__(self, pretrained=True):
        ...
        # 加载预训练的resnet模型,两者的区别在于有没有把第一层conv单独提取
        self.add_mask = True
        (
            self.bg_encoder1,
            self.bg_encoder2,
        ...
        self.fgpool = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, bg_in_data, fg_in_data, mask_in_data=None, mode='val'):
        ...
        # 将前景特征pool后复制
        fg_pool = self.fgpool(fg_in_data_32)  # torch.Size([2, 512, 1, 1])
        fg_in_data = fg_pool.repeat(1, 1, 256, 256)  # torch.Size([2, 512, 256, 256])
